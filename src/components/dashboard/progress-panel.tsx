  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      ...
      <div className="flex items-center justify-between px-1">
        <span className="text-sm font-bold tracking-wider text-muted-foreground uppercase">Progress</span>
        <button
          onClick={handleResetClick}
          title="Reset Progress"
          className="cursor-pointer rounded-full p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-all duration-200 border-none bg-transparent"
        >
      ...
      <div className="rounded-2xl border border-border bg-card p-5 pb-10 shadow-sm flex items-center justify-between gap-5 w-full">
        {/* Left Side: Circular Gauge */}
        <div className="flex flex-col items-center justify-center rounded-xl bg-muted/20 dark:bg-muted/10 border border-border/10 p-4 relative flex-1 min-w-[160px] min-h-[180px]">
          <div className="relative flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="flex items-baseline">
                <span className="text-xl font-extrabold text-foreground">{solvedProblems}</span>
                <span className="text-sm text-muted-foreground font-bold">/{totalProblems}</span>
              </div>
              <span className="text-xs text-muted-foreground font-bold uppercase tracking-wider mt-0.5">Solved</span>
            </div>
          </div>
        </div>

        {/* Right Side: Stacked Difficulty Cards (Vertical stack) */}
        <div className="flex flex-col gap-2 w-[100px] justify-between py-1">
          {/* Easy Card */}
          <div className="flex flex-col items-center justify-center rounded-lg bg-easy/[0.08] border border-easy/20 py-3 px-2 text-center flex-1">
            <span className="text-[11px] font-extrabold uppercase tracking-wider text-easy">Easy</span>
            <span className="text-xs font-extrabold text-foreground mt-0.5">
              {solvedEasy}<span className="text-muted-foreground/80 text-[10px] font-semibold">/{totalEasy}</span>
            </span>
          </div>

          {/* Medium Card */}
          <div className="flex flex-col items-center justify-center rounded-lg bg-medium/[0.08] border border-medium/20 py-3 px-2 text-center flex-1">
            <span className="text-[11px] font-extrabold uppercase tracking-wider text-medium">Med.</span>
            <span className="text-xs font-extrabold text-foreground mt-0.5">
              {solvedMedium}<span className="text-muted-foreground/80 text-[10px] font-semibold">/{totalMedium}</span>
            </span>
          </div>

          {/* Hard Card */}
          <div className="flex flex-col items-center justify-center rounded-lg bg-hard/[0.08] border border-hard/20 py-3 px-2 text-center flex-1">
            <span className="text-[11px] font-extrabold uppercase tracking-wider text-hard">Hard</span>
            <span className="text-xs font-extrabold text-foreground mt-0.5">
              {solvedHard}<span className="text-muted-foreground/80 text-[10px] font-semibold">/{totalHard}</span>
            </span>
          </div>
        </div>
      </div>
